...
const SUBSCRIBE_CENTURIA = gql`
  mutation subscribeCenturia($centuriaId: String!) {
    subscribeCenturia(centuriaId: $centuriaId) {
      id
      centuria {
        name
        semester
...
const SelectCenturia = ({ user: { loading, data, refetchData } }) => {
      ...
      <div className="select-centuria">
        ...
        <CardContent>
          {centurias &&
            centurias.map(item => {
              const isSelected =
                data && data.centuria && data.centuria.name === item.name;
              return (
                <Button
                  key={item.name}
                  variant="contained"
